    ...
    def post(self, request, *args, **kwargs):
        ...
        if not book_id:
            ...
            for author_name in book_authors:
                first_name, last_name, middle_name = author_name.split()
                author_query_data = [first_name, last_name, middle_name]
                with connection.cursor() as cursor:
                    cursor.execute(author_query, author_query_data)
            ...
            with connection.cursor() as cursor:
                cursor.execute(copy_query, copy_query_data)
                response = {"BookID": book_id, "LibraryID": library_id, "BookCondition": book_condition}
                return JsonResponse(response, safe=False)	
